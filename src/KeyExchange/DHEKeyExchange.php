...
class DHEKeyExchange implements KeyExchangeInterface
{
     ...
     */
    public function setDHParameters(string $p, string $g, string $serverPublicKey): void
    {
        $this->p = $p;
        $this->g = $g;
    ...
    {
        if ('' === $this->p || '' === $this->g || '' === $this->serverPublicKey) {
    ...
    {
        if ('' === $this->clientPrivateKey || '' === $this->serverPublicKey || '' === $this->p) {
    ...
    private function hexToBin(string $hex): string
    {
        ...
        for ($i = 0; $i < strlen($hex); $i += 2) {
            $result .= chr((int) hexdec(substr($hex, $i, 2)));
        }
